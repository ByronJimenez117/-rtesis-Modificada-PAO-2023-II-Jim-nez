...
//Número 1: soporte antebrazo, Número 2: soporte mano, Número 3: mano, Número 4: soporte pulgar, Número 5: dedo, Número 6: soporte mano con inclinacion
...
rotate([0,angulo_elevacion,0])
translate([-valor+0.99,0,-20+0.17])
base_soporte(valor1+30,10,5);

}
elevacion=9.37;
translate([-8.4+valor+valor1,30,1.8])
empalme(4,50,90,90,0);
ajuste= 0.9340+0.387/2;

translate([+14.1142-5.58177+ajuste+-valor-valor1-15/1.78,0,3.75+elevacion-0.0475+10.5-1.60+2.0316-5.13714])
rotate([0,20,0])
soporte_tuerca();

translate([+14.1142+ajuste+-valor-valor1-45/1.78,0,3.75+elevacion+14.950+0.05-5.13714])
rotate([0,20,0])
soporte_tuerca();
}
}
...
module soporte_thumb(){
 difference(){
        union(){
            ...
            //cambiar el valor de 10
            base_thumb(largo2,5,forearm_width/5);
            //translate([0.49528*largo2-52.75,0,0.5*desfase_vertical-5])
            translate([0.49528*largo2-52.75 +0.32602*hand_length-25.1041,0.31575*hand_length-24.31301,0.5*desfase_vertical-5])
            union(){
                translate([75.7,26.7,5])
...
module dedo_thumb_2(diametro, largo,espesor,inclinacionX,inclinacionY,inclinacionZ){
rotate([inclinacionX,inclinacionY,inclinacionZ])
difference(){
union(){
translate([0,0,espesor/2])
...
module soporte_resorte_thumb_2(){
rotate([-90,0,0])
union(){ 
translate([-1.5,-1.5,1.75])
cube([3,3,3]);
rotate([0,0,45])
cylinder(h=3,d1=9.65,d2=3.5,$fn=4);
translate([-5.5,-1.5,4.5])
cube([7,3,3]);
...
module tercer_modulo()  {
    difference(){
        union(){ 
            base_soporte(18,10, 5);
            translate([-16, 0, 0])
            base_soporte(valor1+2.5+17, 10, 5);
            translate([-5,0,5])
            rotate([-90,180,0])
            scale([1.35,1.5,1.45])
            soporte_resorte_thumb_2 ();
        ...
        translate([-valor/3.171428571-15, 0, 3.75])
        soporte_tuerca();
        
        translate([-valor/1.37037037-15, 0, 3.75])
        soporte_tuerca(); 
        
    }
}
module empalme_inclinado(){

union(){
}
rotate([0,angulo_elevacion,0])
rotate([0,0,180])
translate([100+2.5+17-4.2+0.5,-25,5.2-20-4+2.75])

empalme(6.5,60,90,270,180);

}

module hand_stag_straight_inclined() {
    difference() {
        union() {
            tercer_modulo() ; 
            translate([-2.5-2.5-17, 0, -0.01])
            hand_stag_straight();
        }
            translate([-27.51+7, 6, 3.75])
            empalme_inclinado();
    }
}

//// Definir la base cuadrada
module base() {
    square([25, 25]);
}

// Crear el prisma principal
module prisma_principal() {
    linear_extrude(height = 10) {
        base();
    }
}

// Crear el prisma más pequeño para restar
module prisma_pequeno() {
    translate([3.5, 3.5, 0]) {  // Ajustar la posición para centrar el hueco
        cube([100,18.5, 5]);
    }
}

// Operación de diferencia para restar el prisma más pequeño del prisma principal
module prisma_con_hueco() {
    difference() {
        prisma_principal();
        prisma_pequeno();
    }
...
module triangule_prism(b,d,h , ht , sc) {
    linear_extrude(height= ht , scale=sc)
    polygon(points= [[0,0], [h,0] , [d,h ] ]  );
    
    }
ancho_triangle= 5;
module bottom_soporte() {
        prisma_con_hueco();{

        union() {
           translate([21-1, 25, 10]) 
rotate([90,0,0])
triangule_prism(15,2,ancho_triangle,25 ,1);
            
            translate([17-2, 25, 10]) 
rotate([90,0,0])
triangule_prism(15,2,ancho_triangle,25 ,1);
            
rotate([90,0,180])
translate([-3.5-1.15, 10, 12.5]) 
scale([1.35,1.5,1.45])
soporte_resorte_thumb_2 ();
        }
    }
...
module bottom_soporte_final() {
    difference() {
        union() {
        bottom_soporte();
        }
        translate([19+1.25, 12.5, 6.25]) 
        soporte_tuerca(); 
     }
}
...
// Crear el prisma secundario
module prisma_secundario() {
    linear_extrude(height = 10) {
        base_2();
    }
}
module prisma_terciario() {
...
module top_soporte_final() {
    difference() {
        union() {
        top_soporte();
            
translate([20-5-2.5, 25, 10]) 
rotate([90,0,0])
triangule_prism(15,2,ancho_triangle,25 ,1); 
            
translate([25-2.5, 25, 10]) 
rotate([90,0,0])
triangule_prism(15,2,ancho_triangle,25 ,1); 
        
translate([25-2.5-5, 25, 10]) 
rotate([90,0,0])
triangule_prism(15,2,ancho_triangle,25 ,1);  
            }

       translate([19+offset_2, 12.5, 5]) 

        soporte_tuerca();
            
           translate([14+1, 25, 10])
            rotate([90,0,-90])
            cube([25, 5, 5]);
            
            translate([31-1-0.025, 25, 10])
            rotate([90,0,-90])
            cube([25, 5, 5]);
     }
}
...
if(pieza==1){    soporte_antebrazo();
   ...
   if(mano=="derecha"){ overhand_pieza_derecha();}else{
    ...
    hand_stag_straight_inclined(); 
    }else if(pieza==7){
bottom_soporte_final();
} 
else if(pieza==8){
 top_soporte_final();
}
